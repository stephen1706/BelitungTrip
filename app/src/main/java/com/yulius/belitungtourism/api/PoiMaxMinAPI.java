...
public class PoiMaxMinAPI extends BaseAPI {
    private OnResponseListener mOnResponseListener;

    public PoiMaxMinAPI(Context context){
        super(context);

        TAG = "PoiMaxMinAPI";
    }

    public void requestPoiMaxMin(final String days){
        String url = API_POI_MAX_MIN_URL;
        Log.v("TEST Request (PoiMaxMinAPI)", "request poi max min");

        StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                if(BuildConfig.DEBUG) {
                    Log.v("TEST Response (PoiMaxMinAPI)", response);
                }
                ...
                try {
                    ...
                    Log.e(TAG, "error parsing json in POI MAX MIN, " + e.getMessage());
                }
            ...
            public void onErrorResponse(VolleyError volleyError) {
                if(mOnResponseListener!=null){
                    ...
                    Log.v("TEST Response (PoiMaxMinAPI)", "error : " + volleyError.getMessage());
                }
            }
        }){
            @Override
